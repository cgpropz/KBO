  ...
  return (
    <section className="nfl-player-page">
      ...
      <div className="nfl-player-header">
        <div className="nfl-player-avatar">{currentRow.imageUrl ? <img src={currentRow.imageUrl} alt={player} loading="lazy" /> : initials(player)}</div>
        <div className="nfl-player-title"><h1>{player}<span>{currentRow.position}</span></h1><p>{currentRow.team} vs {currentRow.opponent}</p></div>
        <div className="nfl-player-prop-pill"><span>{formatValue(currentRow.line)} {currentRow.prop}</span><b className={isOver ? 'over' : 'under'}>{isOver ? 'OVER' : 'UNDER'} {currentRow.score.toFixed(1)}</b></div>
      </div>
      ...
      <div className="nfl-player-summary">
        ...
        <div><small>MODEL</small><strong>{formatValue(currentRow.projection)}</strong></div>
      ...
      <div className="nfl-player-dates">{gameDates.map((date, index) => <span key={`${currentRow.id}-date-${index}`}>{date}</span>)}</div>
    </section>
  )
}
